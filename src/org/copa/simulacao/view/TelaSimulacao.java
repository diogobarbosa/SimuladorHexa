...
public class TelaSimulacao {
	...
	public void construirTelaSimulacao(JFrame framePrincipal) {
		...
		// Realiza o cadastro de times (Via properties, enquanto não há banco de dados.
		// Para fins de teste)
		...
		for (Grupo grupo : listaGrupos) {
			...
			panelGrupo.setBackground(Color.getHSBColor(328, 1.0F, 0.49F));
			...
			for (Partida partida : grupo.getListaPartidas()) {
				...
				JLabel labelPartida = new JLabel(
						" " + time.getNome() + " vs " + timeAdversario.getNome() + ": " + partida.getResultado());
				panelGrupo.add(labelPartida);
				labelPartida.setForeground(Color.WHITE);

			}

			Integer timesPorGrupo = listaGrupos.get(0).getListaTimes().size();
			Integer quantidadePartidasFaseGrupos = Utils.fatorial(timesPorGrupo)
					/ (Utils.fatorial(timesPorGrupo - 2) * 2);
		...
		// Organiza os times do grupo por quantidade de pontos (E desempate por vitória
		// e saldo de gols).
		...
		for (Partida partida : grupoVendedoresOitavaFinal.getListaPartidas()) {
			...
			panelOitavaFinal.setBackground(Color.getHSBColor(328, 1.0F, 0.49F));
			...
			JLabel labelPartidaOitavaFinal = new JLabel(
					" " + time.getNome() + " vs " + timeAdversario.getNome() + ": " + partida.getResultado());
			labelPartidaOitavaFinal.setForeground(Color.WHITE);
		...
		Grupo grupoVencedoresQuartaFinal = simuladorCopa
				.realizarProximaEtapa(grupoVendedoresOitavaFinal.getListaTimes(), Fase.QUARTA_FINAL);
		...
		for (Partida partida : grupoVencedoresQuartaFinal.getListaPartidas()) {
			...
			panelQuartaFinal.setBackground(Color.getHSBColor(328, 1.0F, 0.49F));
			...
			JLabel labelPartidaQuartaFinal = new JLabel(
					" " + time.getNome() + " vs " + timeAdversario.getNome() + ": " + partida.getResultado());
			labelPartidaQuartaFinal.setForeground(Color.WHITE);
		...
		Grupo grupoVencedoresSemiFinal = simuladorCopa.realizarProximaEtapa(grupoVencedoresQuartaFinal.getListaTimes(),
				Fase.SEMI_FINAL);
		...
		for (Partida partida : grupoVencedoresSemiFinal.getListaPartidas()) {
			...
			panelSemiFinal.setBackground(Color.getHSBColor(328, 1.0F, 0.49F));
		...
		panelFinal.setBackground(Color.getHSBColor(328, 1.0F, 0.49F));
		...
		JLabel labelPartidaFinal = new JLabel(" " + time.getNome() + " vs " + timeAdversario.getNome() + ": "
				+ vencedorFinal.getListaPartidas().get(0).getResultado());
		labelPartidaFinal.setForeground(Color.WHITE);
